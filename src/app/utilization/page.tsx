...
export default function UtilizationPage() {
    ...
    const renderContent = () => {
        if (error) {
            return (
                <motion.div 
                    ...
                    transition={{ duration: 0.3 }}
                >
                    <h3 className="font-medium mb-2">连接错误</h3>
                    <p>{error}</p>
                    <button 
                        className="mt-2 px-3 py-1 bg-red-100 hover:bg-red-200 rounded-lg text-sm"
        ...
        if (snapshot === null) {
            return (
                <motion.div 
                    className="p-6 bg-gray-50 rounded-lg flex items-center justify-center min-h-[200px]"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ duration: 0.5 }}
                >
                    <div className="text-center">
                        <Icon.Loader className="h-8 w-8 text-gray-400 animate-spin mb-2 mx-auto" />
                        <p className="text-gray-500">正在加载 GPU 数据...</p>
                    </div>
                </motion.div>
            );
        }
        ...
        if (snapshot.length === 0) {
            return (
                <motion.div 
                    className="p-6 bg-gray-50 rounded-lg text-center min-h-[200px] flex flex-col items-center justify-center"
                    initial={{ opacity: 0, scale: 0.95 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.4 }}
                >
                    <Icon.AlertCircle className="h-8 w-8 text-amber-500 mb-2 mx-auto" />
                    <h3 className="font-medium mb-1">未检测到 GPU 设备</h3>
                    <p className="text-gray-500 text-sm">未找到支持的 AMD GPU 设备，请确认驱动已正确安装</p>
                </motion.div>
            );
        }
        
        // 显示 GPU 和显存利用率
        return <PerformanceMetricsGrid device={selectedDeviceData} />;
    };
        ...
        <div className="flex flex-col p-4 md:p-6 lg:p-8">
            ...
            >
                <div className="flex items-center mb-6">
                    ...
                    <h2 className="text-lg font-medium text-gray-800">GPU 利用率指标</h2>
                </div>
